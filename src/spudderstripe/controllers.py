...
class StripeController(object):
    ...
    def accept_payment(self, payment_description, card_token, amount):
        customer, error = spudderstripe.api_utils.create_customer(card_token, payment_description)
        ...
        token, error = spudderstripe.api_utils.create_token(customer, self.stripe_user.access_token)
        if error:
            logging.error('Error during token creation')
            logging.error(error)
            return {'success': False}

        charge, error = spudderstripe.api_utils.make_charge(amount, token, payment_description, self.stripe_user.access_token)
        if error:
